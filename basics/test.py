...
def test(
    ...
    iouv = torch.linspace(0.5, 0.95, 10).to(
        device
    )  # iou vector for mAP@0.5:0.95 zjq
    niou = iouv.numel()
    ...
    if not training:
        ...
        dataloader = create_dataloader_sr(
            data[task],
            imgsz,
            batch_size,
            gs,
    ...
    for batch_i, (img, ir, targets, paths, shapes) in enumerate(
        ...
        nb, chn, height, width = img.shape  # batch size, channels, height, width
        ...
        with torch.no_grad():
            ...
            try:
                out, train_out = model(
                    img, ir, input_mode=input_mode
                )  # zjq inference and training outputs
            except:
                out, train_out, _ = model(
                    img, ir, input_mode=input_mode
                )  # zjq inference and training outputs
            ...
            breakpoint()
            # out_2 = weighted_boxes(out, image_size=imgsz, conf_thres=conf_thres, iou_thres=iou_thres, labels=lb, multi_label=True)
        ...
        for si, pred in enumerate(out):
            ...
            if (
                ...
                if (
                    wandb_logger.current_epoch % wandb_logger.bbox_interval
                    == 0
                ):
                    box_data = [
                        {
            ...
            if save_json:
                ...
                image_id = (
                    int(path.stem) if path.stem.isnumeric() else path.stem
                )
        ...
        if plots:  # and batch_i < 3: #zjq
            ...
            if input_mode == "IR":
                Thread(
                    target=plot_images,
                    args=(ir, output_to_target(out), paths, f, names),
                    daemon=True,
                ).start()
            else:
                Thread(
    ...
    if not training:
        print(
            "Speed: %.3f/%.3f/%.3f ms inference/NMS/total per %gx%g image at batch-size %g"
            % t
        )
    # Plots
    if plots:
    ...
    if save_json and len(jdict):
        ...
        anno_json = (
            "../coco/annotations/instances_val2017.json"  # annotations json
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        default="small_EDSR_fold1.pt",
        help="model.pt path(s)",
    )
    parser.add_argument(
        "--data", type=str, default="data/SRvedai.yaml", help="*.data path"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--input_mode", type=str, default="RGB+IR"
    )  # RGB IR RGB+IR RGB+IR+fusion
    parser.add_argument(
        "--conf-thres",
        type=float,
        default=0.001,
