...
from basics.utils.general import (
...
from basics.utils.plots import (
    plot_images,
    plot_labels,
    plot_results,
    plot_evolution,
...
from basics.utils.torch_utils import (
    ModelEMA,
    select_device,
    intersect_dicts,
...
from basics.utils.wandb_logging.wandb_utils import (
    WandbLogger,
    check_wandb_resume,
)

from basics.utils.load_ckpt import remap_pretrained_keys_swin
...
def train(hyp, opt, device, tb_writer=None):
    ...
    pretrained = (
        weights.endswith(".pt")
        if opt.simmim_pretrained == False
        else weights.endswith(".pth")
    )
    ...
    if pretrained:
        ...
        if opt.simmim_pretrained:
            ...
            # print("BEFORE")
            # print(model_dict["encoder.layers.0.blocks.0.attn.qkv.weight"])
            # print(state_dict["encoder.layers.0.blocks.0.attn.qkv.weight"])
            state_dict = remap_pretrained_keys_swin(model_dict, state_dict, logger)
            model.load_state_dict(state_dict, strict=False)
            # print("AFTER")
            # print(model.state_dict()["encoder.layers.0.blocks.0.attn.qkv.weight"])
            logger.info(
            ...
            exclude = (
                ["anchor"]
                if (opt.cfg or hyp.get("anchors")) and not opt.resume
                else []
            )  # exclude keys
    ...
    nl = model.detect[
        -1
    ].nl  # number of detection layers (used for scaling hyp['obj'])   #* changed model.model[-1]   to model.detect
    imgsz, imgsz_test = [
    ...
    if rank in [-1, 0]:
        ...
        testloader = create_dataloader(
            ...
            batch_size,
            gs,
            opt,  # testloader     #*changed  batch_size//2
            hyp=hyp,
    ...
    def Log_UP(K_min, K_max, epoch):
        ...
        return (
            torch.tensor([math.pow(10, Kmin + (Kmax - Kmin) / epochs * epoch)])
            .float()
            .cuda()
        )
    ...
    for epoch in range(
        ...
        for i, (
            ...
            if down_factor > 1:
                imgs = F.interpolate(
                    image,
                    size=[i // down_factor for i in image.size()[2:]],
                    mode="bilinear",
                    align_corners=True,
                )
                ...
                irs = F.interpolate(
                    ir_image,
                    size=[i // down_factor for i in ir_image.size()[2:]],
                    mode="bilinear",
                    align_corners=True,
                )
            ...
            if opt.multi_scale:
                ...
                if sf != 1:
                    ...
                    irs = F.interpolate(
                        irs, size=ns, mode="bilinear", align_corners=False
                    )  # zjq
            ...
            with amp.autocast(enabled=cuda):
                ...
                if (
                    opt.super
                ):  # and not opt.attention and not opt.super_attention:
                    pred, output_sr, _ = model(
                        imgs, irs, opt.input_mode
                    )  # forward #zjq
                ...
                loss, lbox, lobj, lcls = compute_loss(
...
if __name__ == "__main__":
    ...
    #############################
    parser.add_argument(
        "--simmim_pretrained", default=False, help="Use simMIM or not"
    )
    parser.add_argument(
        "--weights", type=str, default="", help="initial weights path"
    )
    parser.add_argument(
        "--cfg", type=str, default="models/model.yaml", help="model.yaml path"
    )  # yolov5s
    parser.add_argument(
        "--super", default=False, action="store_true", help="super resolution"
    )
    parser.add_argument(
        "--data",
        type=str,
        default="models/data_vedai.yaml",
        help="data.yaml path",
    )
    parser.add_argument(
        "--hyp",
        type=str,
        default="models/hyp.scratchs.yaml",
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="3 4 16 midfusion1:64 midfusion2,3:128 midfusion4:256",
    )  # *changed from default to match SAM
    parser.add_argument(
        "--input_mode",
        type=str,
        default="RGB+IR",
        help="RGB IR RGB+IR(pixel-level fusion) RGB+IR+fusion(feature-level fusion)",
    )
    parser.add_argument(
        "--batch-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="train image sizes,if use SR,please set 1024",
    )
    parser.add_argument(
        "--test_img_size", type=int, default=512, help="test image sizes"
    )
    parser.add_argument(
        "--hr_input",
        default=True,
        action="store_true",
        help="high resolution input(1024*1024)",
    )  # if use SR,please set True
    parser.add_argument(
    ...
    parser.add_argument(
        "--project",
        default="outputs_SAM/yoloSAM_v2_p6_multiL_RGBIR_glob_CF_v2_ViT1/run/train",
    ...
    opt.global_rank = (
        opt.local_rank
    )  # int(os.environ['LOCAL_RANK']) if 'RANK' in os.environ else -1
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ckpt = (
            ...
            if isinstance(opt.resume, str)
                ...
                "/home/bbahaduri/sryolo/outputs_SAM/yoloSAM_v2_p6_multiL_RGBIR_glob_CF_v2_final2/run/train/exp9/"
            )
    ...
    if opt.local_rank != -1:
        ...
        dist.init_process_group(
            backend="nccl", rank=opt.local_rank, world_size=2
        )  # init_method='env://')  # distributed backend  #*rank , environment
        assert (
